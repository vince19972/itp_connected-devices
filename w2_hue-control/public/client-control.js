var url = '128.122.151.177';           // the hub IP address
var username = '1F26-hiR9LXH8T-rJit9V-DQXstDjfcDwJjgbGCB';
var usernameField;
var addressField;
  ...
  for (property in state) {     // iterate over  properties in state object
    ...
    switch (property) {         // handle the cases you care about
      ...
      case 'effect':
        myInput = createInput();  // an input for the on property
        myInput.attribute('type', 'checkbox');    // make this input a checkbox
        myInput.attribute('checked', state.effect);	  // is called 'checked'
        myInput.mouseClicked(changeProperty); // set the mouseClicked callback
        break;
      case 'alert':
        myInput = createInput();  // an input for the on property
        myInput.attribute('type', 'checkbox');    // make this input a checkbox
        myInput.attribute('checked', state.alert);	  // is called 'checked'
        myInput.mouseClicked(changeProperty); // set the mouseClicked callback
        break;
    }
...
function changeProperty() {
  ...
  if (thisControl === 'effect') {
    payload[thisControl] = event.target.checked ? 'colorloop' : 'none';
  }
  if (thisControl === 'alert') {
    payload[thisControl] = event.target.checked ? 'select' : 'lselect';
  }
